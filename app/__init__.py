...
from NYT_Flashbacks.app import db_handling, api_handling
...
def home():
    if 'index' not in session:
        ...
        session['next_event'] = api_handling.getEvent(random.randint(1, 12), random.randint(0, 200), int(session['curr_event'][0]))
    lost = False
    index = session['index']
